...
function affRecherche_bbc($Messages, $Mots)
	...
	foreach($Messages as $M)
		...
		if (preg_match("#^Rapport#", $Tableau['Type']))
			...
			$Sortie .= fonction("temps", array("TvD", $M['date'], "#N2/#M2/#A2 à #H2:#I2"));
	...
	$Sortie .= " le ".date("d/m/Y à H:i:s");
	$Sortie .= " avec la requête [b]".$GLOBALS['recherche']."[/b]";
	if ($GLOBALS['grouper'])	{ $Sortie .= " groupée par cible"; }
	else					{ $Sortie .= " sans groupement"; }
	if ($GLOBALS['limite'] !== FALSE)
		{
		if ($GLOBALS['heures'])		{ $Sortie .= " limitée à ".$GLOBALS['limite']." heures"; }
		else if ($GLOBALS['jours'])	{ $Sortie .= " limitée à ".$GLOBALS['limite']." jours"; }
		}
